...
sozluk = {
    ...
    "": "Veri saklamak için kullanılan bellek birimi.",
    "Döngü": "Belirli bir kod bloğunu tekrar tekrar çalıştıran yapı."
}

# Başlık
st.title("Basit Sözlük Uygulaması")

# Kelimeleri ve tanımları göster
st.header("Kelimeler ve Tanımları")
for kelime, tanim in sozluk.items():
    if st.button(kelime):
        st.write(tanim)
...
yeni_tanim = st.text_input("Yeni Tanım")
if st.button("Ekle"):
    if yeni_kelime and yeni_tanim:
        sozluk[yeni_kelime] = yeni_tanim
        st.success(f"'{yeni_kelime}' kelimesi sözlüğe eklendi!")
    else:
        st.error("Lütfen hem kelime hem de tanım girin.")
